...
export const setTokens = (
  ...
  Cookie.set("refreshToken", refreshToken, { ...options, expires: 7 });
  Cookie.set("nome", nome, options);
  Cookie.set("email", email, options);
  Cookie.set("cpf", cpf, options);
  Cookie.set("id", id.toString(), options);
};
...
export const refreshAccessToken = async (
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_ROUTE_URL}/auth/refresh`,
      {
        method: "POST",
        headers: {
...
export const isAuthenticated = async () => {
  ...
  const checkAuth = async (): Promise<{
    ...
    if (!refreshToken) return { isAuthenticated: false, accessToken: null };
